...
export const getMorePosts = async (page_number) => {
  try {
      if(page_number<100){
        const response = await axios.get(`${BASE_URL}/accounts?page=${page_number}`);
        return response.data;
      }
...
export const getPostDetails = async (slug) => {
  try {
    const response = await axios.get(`${BASE_URL}/accounts/details/${slug}`);
    return response.data;
  } catch (error) {
    throw new Error("Failed to fetch post details data");
  }
};
export const searchPosts = async (search_value) => {
  try {
    const response = await axios.get(`${BASE_URL}/accounts/search/${search_value}`);
    return response.data;
  } catch (error) {
    throw new Error("search failed");
  }
